...
# Computes scores for each loaded parameter in the list of dictionaries and returns a new dictionary of SCORES and STATES!

# I decided to do both because the scored will be used to calculate the final surf rating, but the states will help me understand what the numbers mean.
...
# Getting the necessary data from the 'hour' dictionary, we need the swell direction and period of primary and secondary swells.
        swell_direction = float(hour.get("swellDirection", 0))
...
# Calculating the difference in direction between the secondary and primary swells hehe.
        
        directional_difference = abs(swell_direction - secondary_swell_direction) # we cant have negative directions lol
...
# Alright here we calculate the difference between periods
        
        period_difference = abs(swell_period - secondary_swell_period) # we cant have negative periods lol
...
# Here we calculate the the difference in size as a perfecntage of each 
...
# finally! addint up the scores.
        
        total_disruption_score_raw = directional_score + period_score + size_score

# Normalisatiooooooooooooooon
        
        normalized_score = round(1 + (total_disruption_score_raw - 3) * (4/9))

# Assigning the corresponding state to the score, we need to make sense of these number please !
...
########## SWELL X TIDE SCORES -- hardddd ##########

# Extracting tide data from the hour
        
        tide_height = float(hour.get("tideHeight", 0))

# Categorising tide height
        
        if -0.1 <= tide_height < 0.3:
            tide_state = "LOW"
        elif 0.3 <= tide_height < 0.6:
            tide_state = "MID"
        else:  # tide_height >= 0.6
            tide_state = "HIGH"

# Logic. Probably an easiest way to do it because of the redundancy, but hey this should work!
        
        if scores['swell_height'] == 5:
            states['swell_x_tide'] = "Close Out"
            scores['swell_x_tide'] = 0
        else:
            if tide_state == "LOW":
                if scores['swell_height'] >= 4:
                    states['swell_x_tide'] = "Close Out"
                    scores['swell_x_tide'] = 0
                elif scores['swell_height'] <= 2:
                    states['swell_x_tide'] = "Ideal"
                    scores['swell_x_tide'] = 2
                else:
                    states['swell_x_tide'] = "Working"
                    scores['swell_x_tide'] = 1
            elif tide_state == "MID":
                if scores['swell_height'] == 3:
                    states['swell_x_tide'] = "Ideal"
                    scores['swell_x_tide'] = 2
                else:
                    states['swell_x_tide'] = "Working"
                    scores['swell_x_tide'] = 1
            elif tide_state == "HIGH":
                if scores['swell_height'] == 1:
                    states['swell_x_tide'] = "Not Breaking"
                    scores['swell_x_tide'] = 0
                elif scores['swell_height'] == 4:
                    states['swell_x_tide'] = "Ideal"
                    scores['swell_x_tide'] = 2
                else:
                    states['swell_x_tide'] = "Working"
...
# Read the hourly forecast data and save to varriable data (instead of csv, easier, its not memory heavy)

data = read_hourly_forecast()

# If there's data, process and print each hour's scores and states. i.e. if its working do it !

if data:
    for idx, hour in enumerate(data): #idx being abreviation of index
        scores, states = parameter_scores(hour) #we initialised this just at the start

# Extracting and formatting the time
        
        time_string = hour.get("time", "")
        try:
...
# Format the datetime object to display only the hh:mm

            formatted_time = time_obj.strftime("%H:%M")
        except ValueError:
            formatted_time = f"Hour {idx + 1}"
